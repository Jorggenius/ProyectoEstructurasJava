...
public class ControlVistaDevoluciones {
    ...
    public void interaccionCola(Tiquete tiquete){
    ...
    public Lista<Tiquete> tiquetesActuales(){
        Lista<Tiquete> tiquetes = new Lista<>();
        for (int i = 0; i < caseta.getEmpresa().getTiquetes().size(); i++) {
            if(caseta.getEmpresa().getTiquetes().get(i).getViaje().getFechaSalida().isAfter(LocalDate.now())){
                tiquetes.add(caseta.getEmpresa().getTiquetes().get(i));
            }
        }
        return tiquetes;
    }
