...
public class ControlVistaTerminal {
        ...
        for (int i = 0; i < casetas.length; i++) {
            for (int j = 0; j < casetas[i].length; j++) {
                for (int k = 0; k < casetas[i][j].getEmpresa().getBuses().size(); k++) {
                    if (casetas[i][j].getEmpresa().getBuses().get(k).getPlaca().equalsIgnoreCase(placa)) {
                        return true;
                    }
                }
            }
        ...
        for (int i = 0; i < casetas.length; i++) {
            for (int j = 0; j < casetas[i].length; j++) {
                for (int k = 0; k < casetas[i][j].getEmpresa().getTiquetes().size(); k++) {
                    if(casetas[i][j].getEmpresa().getTiquetes().get(k).getCliente().getId() == cliente.getId()){
                        tiquetesCliente.add(casetas[i][j].getEmpresa().getTiquetes().get(k));
                    }
                }
            }
        }
    ...
    public Lista<Reserva> obtenerReservas(Cliente cliente) {
        Lista<Reserva> reservasCliente = new Lista<>();
        for (int i = 0; i < casetas.length; i++) {
            for (int j = 0; j < casetas[i].length; j++) {
                for (int k = 0; k < casetas[i][j].getEmpresa().getReservas().size(); k++) {
                    if(casetas[i][j].getEmpresa().getReservas().get(k).getCliente().getId() == cliente.getId()){
                        reservasCliente.add(casetas[i][j].getEmpresa().getReservas().get(k));
                    }
                }
            }
        }
    ...
    public Lista<Redencion> obtenerRedenciones(Cliente cliente){
         Lista<Redencion> redencionesCliente = new Lista<>();
        for (int i = 0; i < casetas.length; i++) {
            for (int j = 0; j < casetas[i].length; j++) {
                for (int k = 0; k < casetas[i][j].getEmpresa().getRedenciones().size(); k++) {
                    if(casetas[i][j].getEmpresa().getRedenciones().get(k).getCliente().getId() == cliente.getId()){
                        redencionesCliente.add(casetas[i][j].getEmpresa().getRedenciones().get(k));
                    }
                }
            }
        }
        return redencionesCliente;
    }
}
