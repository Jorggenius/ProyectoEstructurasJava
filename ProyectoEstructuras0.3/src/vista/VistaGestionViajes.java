...
public class VistaGestionViajes extends javax.swing.JFrame {
    ...
    private void llenarTabla() {
        DefaultTableModel model = new DefaultTableModel(new String[]{"Origen",
            "Destino", "FechaCreacion", "FechaSalida", "HoraSalida", "FechaLlegada", "HoraLlegada",
            "Bus", "ValorUnidad", "id"}, controlGV.getCaseta().getEmpresa().getViajes()
                .size());
        tablaViajes.setModel(model);
        TableModel modelViaje = tablaViajes.getModel();
        for (int i = 0; i < controlGV.getCaseta().getEmpresa().getViajes().size(); i++) {
            Viaje viaje = controlGV.getCaseta().getEmpresa().getViajes().get(i);
            ...
            modelViaje.setValueAt(viaje.getFechaSalida(), i, 3);
            modelViaje.setValueAt(viaje.getHoraSalida(), i, 4);
            modelViaje.setValueAt(viaje.getFechaLLegada(), i, 5);
            modelViaje.setValueAt(viaje.getHoraLlegada(), i, 6);
            modelViaje.setValueAt(viaje.getBus(), i, 7);
            modelViaje.setValueAt(viaje.getValorUnidad(), i, 8);
...
//                    txtIdViaje.setText((String)tablaViajes.getValueAt(tablaViajes.getSelectedRow(), 8));
                    txtIdViaje.setText(String.valueOf(tablaViajes.getValueAt(tablaViajes.getSelectedRow(), 9)));
                }
            }
    ...
    private void btnIngresarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnIngresarActionPerformed
        ...
        try {
            ...
            LocalDate fechaSalida = fecha3.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
            ...
            if (controlGV.disponibilidadBus(fechaSalida,horaSalida,bus)) {
                Viaje viaje = new Viaje(origen, destino, fechaCreacion, fechaSalida, horaSalida,
                ...
                JOptionPane.showMessageDialog(null, "El bus No esta disponible para la fecha");
            }
        } catch (IdExiste e) {
